...
void test_lis_sim(int x, int y, lis_sim &X,int nrRez,int nrLimba,int nrSound)
{
    ...
    while(true)
    {
        ...
        if(coord_y_istoric>=688*rez4)
        {
            ...
            bar(950*rez4,500*rez4,1280*rez4,720*rez4);
        }

        if(x>=15*rez4 && x<=65*rez4 && y>=665*rez4&&y<=710*rez4)  ///undo
        {
            if(nrSound==1)
                PlaySound(TEXT("press1.wav"), NULL, SND_ASYNC);
        ...
        if(x>=93*rez4&&x<=200*rez4 && y>=511*rez4&&y<=518*rez4)
        {
            ...
            if(nrLimba==0)
                outtextxy(370*rez4,coord_y_consola,"Position:");
            else outtextxy(370*rez4,coord_y_consola,"Pozitie:");
            int pozitie=0;
            while(true)
            {
                c=(char)getch();
                if(c==13)
                {
                    ...
                    {
                        setfillstyle(SOLID_FILL,BLACK);
                        bar(478*rez4,473*rez4,643*rez4,489*rez4);
                        break;
                    }
                }
                if(c>=48 && c<=57)
            ...
            setfillstyle(SOLID_FILL,BLACK);
            bar(5*rez4,3*rez4,1272*rez4,456*rez4);
            bar(695*rez4,500*rez4,950*rez4,720*rez4);
            if(rez4==0.5)
                settextstyle(3,HORIZ_DIR,1);
            if(rez4==1)
                settextstyle(3,HORIZ_DIR,1);
        ...
        if(x>=93*rez4&&x<=189*rez4 && y>=540*rez4&&y<=549*rez4)
        {
            if(nrSound==1)
                PlaySound(TEXT("press1.wav"), NULL, SND_ASYNC);
            if(nrLimba==0)
                outtextxy(972*rez4,coord_y_istoric,"eliminare(poz)");
            else outtextxy(972*rez4,coord_y_istoric,"remove(pos)");
            int pozitie=0;
            while(true)
            {
                c=(char)getch();
        ...
        if(x>=91*rez4&&x<=149*rez4 && y>=570*rez4&&y<=576*rez4)
        {
            if(nrSound==1)
                PlaySound(TEXT("press1.wav"), NULL, SND_ASYNC);
            if(nrLimba==0)
                outtextxy(972*rez4,coord_y_istoric,"golire");
            else outtextxy(972*rez4,coord_y_istoric,"empty");
            gg<<"Golire"<<'\n';
            coord_y_istoric+=20*rez4;
            golesteX(X);
            nr_el=0;
        ...
        if(x>=94*rez4&&x<=249*rez4 && y>=600*rez4&&y<=607*rez4)
        {
            if(nrSound==1)
                PlaySound(TEXT("press1.wav"), NULL, SND_ASYNC);
            if(nrLimba==0)
                outtextxy(972*rez4,coord_y_istoric,"creareAleatorie");
            else outtextxy(972*rez4,coord_y_istoric,"randomCreate");
            gg<<"Creare Aleatorie"<<'\n';
            coord_y_istoric+=20*rez4;
