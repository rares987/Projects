...
void desenCasUlt(double rez2,int time)
{
    delay(time);
    ...
    while(true)
    {
        ...
        if(x>=93*rez2&&x<=267*rez2 && y>=511*rez2&&y<=518*rez2)
        {
            ...
            if(L.prim==NULL)
                esteVid=1;
            setfillstyle(SOLID_FILL,BLACK);
            bar(5*rez2,3*rez2,1272*rez2,456*rez2);
            ...
            if(L.nrElemente==0)
            {
                ...
                desenareLDB(L,rez2,2);
            }
            else
            {
                bar(5*rez2,3*rez2,1272*rez2,456*rez2);
                desenareLDB(L,rez2,3);
                delay(1000);
                bar(5*rez2,3*rez2,1272*rez2,456*rez2);
                adaugaInFata(L,numar);
                desenareLDB(L,rez2,1);
            }
        }
